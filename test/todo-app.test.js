...
test("render_item HTML for a single Todo Item", function (t) {
  ...
  document
    .getElementById(id)
    .appendChild(app.render_item(model.todos[0], model, mock_signal));
...
test("render_item HTML without a valid signal function", function (t) {
  ...
  document
    .getElementById(id)
    .appendChild(app.render_item(model.todos[0], model));
...
test("render_footer 1 item left (pluarisation test)", function (t) {
  const model = {
    todos: [{ id: 1, title: "Be excellent to each other!", done: false }],
    hash: "#/", // the "route" to display
  };
  // render_footer view and append it to the DOM inside the `test-app` node:
  document.getElementById(id).appendChild(app.render_footer(model));
...
test("1. No Todos, should hide #footer and #main", function (t) {
  ...
  const main_footer = window.getComputedStyle(
...
test("4. Item: should allow me to mark items as complete", function (t) {
  ...
  t.equal(
    document.querySelectorAll(".toggle")[0].checked,
    false,
...
test('4.1 DELETE item by clicking <button class="destroy">', function (t) {
  ...
  t.equal(
    document.querySelectorAll("button.destroy").length,
    0,
...
test('5.1 Editing: > Render an item in "editing mode"', function (t) {
  ...
  document
    .getElementById(id)
    .appendChild(app.render_item(model.todos[2], model, mock_signal));
  // test that signal (in case of the test mock_signal) is onclick attribute:
  t.equal(
    document.querySelectorAll(".view > label")[0].onclick.toString(),
    mock_signal().toString(),
    "mock_signal is onclick attribute of label"
  );

  // test that the <li class="editing"> and <input class="edit"> was rendered:
  t.equal(
    document.querySelectorAll(".editing").length,
    1,
...
test("5.5 CANCEL should cancel edits on escape", function (t) {
  ...
  elmish.mount(model, app.update, app.view, id, app.subscriptions);
  t.equal(
    document.querySelectorAll(".view > label")[1].textContent,
    model.todos[1].title,
    "todo id 1 has title: " + model.todos[1].title
  );
  // apply empty string to the <input class="edit">:
...
test("7. Clear Completed > should display the number of completed items", function (t) {
  ...
  const completed_count = parseInt(
  ...
  t.equal(
    document.querySelectorAll("clear-completed").length,
    0,
...
test("8. Persistence > should persist its data", function (t) {
  ...
  t.equal(
    localStorage.getItem("todos-elmish_" + id),
